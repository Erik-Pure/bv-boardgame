...
**Föremål spelat på dig (mobil, `/play`):** när någon annan spelar ett föremål med dig som mål syns en **rik toast** (föremålsbild + namn + *«{aktör} spelade på dig»*) baserat på `tableItemPlayReveals` / `reactionItemPlays` — samma synk som brädet. Visas **inte** samtidigt som blocking `SipNoticeCardModal` (t.ex. Sömnmedel). Autoförsvinner efter några sekunder.
